...
##############################################################
# solution 2:
# select just based on connectivity

# for comparable ranking, you need to make the connectivity as
# penalties and change to -1. Doing it as a feature gave me
# non-informative rankings because of how only 1 metric was
# the limiting one by far, and therefore replacement values
# only changed for a handful of meadows with high dPC scores.
##############################################################

out_folder <- 's02_connectivity'
out_dir <- file.path(out_path, out_folder)
dir.create(out_dir)
setwd(out_dir)
...
p1 <- problem(sg, features, cost_column='cost2') %>%
  add_min_set_objective() %>%
  add_relative_targets(targets) %>%
  add_linear_penalties(penalty = penalty_value, data = 'dPCpld01') %>%
  add_linear_penalties(penalty = penalty_value, data = 'dPCpld60') %>%
  add_linear_penalties(penalty = penalty_value, data = 'sink_01') %>%
  add_linear_penalties(penalty = penalty_value, data = 'sink_60') %>%
  add_linear_penalties(penalty = penalty_value, data = 'source01') %>%
  add_linear_penalties(penalty = penalty_value, data = 'source60') %>%
  add_linear_penalties(penalty = penalty_value, data = 'gpfill01') %>%
  add_linear_penalties(penalty = penalty_value, data = 'gpfill60') %>%  
  add_binary_decisions() %>%
  add_gurobi_solver(gap = 0) %>%
  add_gap_portfolio(number_solutions = 10, pool_gap = 0.1)
...
##############################################################
# solution 3:
# consider connectivity and impacts together
# This will give me the top meadows when not considering area
# or meadows already protected.

# I'm still including the dummy_conn because I want the
# solution  to select at least 30% of the meadows.
##############################################################

out_folder <- 's03_nat_conn'
out_dir <- file.path(out_path, out_folder)
dir.create(out_dir)
setwd(out_dir)
